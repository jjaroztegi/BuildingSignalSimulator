...
export async function fetchComponentsByModel(type, model) {
  try {
    ...
    if (!response.ok) {
      console.error(
        `Error fetching details for ${type} ${model}:`,
        `HTTP error! status: ${response.status}`,
      );
      displayError(
        `Error al cargar los detalles del componente ${model}. Por favor, intente de nuevo.`,
      );
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    return await response.json();
...
export async function runSimulation(simulationData) {
  try {
    ...
    if (!simulateResponse.ok) {
      ...
      displayError(
        'Error al ejecutar la simulación. Por favor, revise los datos e intente de nuevo.',
      );
